...
begin
  try
    ...
    if (UserData.nSocketIdx >= 0) and (UserData.nSocketIdx < GATEMAXSESSION) then begin
              ...
              if SessionArray[UserData.nSocketIdx].nPacketIdx = nPacketIdx then begin
                ...
                if (nDataLen >= DEFBLOCKSIZE) then begin
                  if SessionArray[UserData.nSocketIdx].boStartLogon then begin
                    ...
                    if not CheckDefMsg(@DefMsg,@SessionArray[UserData.nSocketIdx]) then
                    begin
      ...
      SessionArray[UserData.nSocketIdx].sSocData:='';   //  if  (UserData.nSocketHandle = SessionArray[UserData.nSocketIdx].nSckHandle)
      end;
    end;//004563B4
...
procedure TFrmMain.ProcessPacket(UserData:pTSendUserData);   //分快发送数据
//004564E4
var
...
begin
  try
    AddMainLogMsg('王者引擎网关...',0);

    AddMainLogMsg('防后门系统启动成功...',2);
...
begin
  ...
  if Conf <> nil then begin
    ...
    IF nMaxClientPacketSize > 4000 then
    nMaxClientPacketSize:=4000;
    ...
    dwHitTime:=Conf.ReadInteger('Cheat','HitTime',dwHitTime);
    nHitErrorMax:=conf.ReadInteger('Cheat','HitErrorMax',nHitErrorMax);
    dwWalkTime:=Conf.ReadInteger('Cheat','WalkTime',dwWalkTime);
    nWalkErrorMax:=conf.ReadInteger('Cheat','WalkErrorMax',nWalkErrorMax);
    dwSpellTime:=Conf.ReadInteger('Cheat','SpellTime',dwSpellTime);
    nSpellErrorMax:=conf.ReadInteger('Cheat','SpellErrorMax',nSpellErrorMax);
    dwTurnTime:=Conf.ReadInteger('Cheat','TurnTime',dwTurnTime);
    nTurnErrorMax:=conf.ReadInteger('Cheat','TurnErrorMax',nTurnErrorMax);
    boCtrlWalkSpeed:=conf.ReadBool('Cheat','CtrlWalkSpeed',boCtrlWalkSpeed);
    boCtrlHitSpeed:=conf.ReadBool('Cheat','CtrlHitSpeed',boCtrlHitSpeed);
    boCtrlTurnSpeed:=conf.ReadBool('Cheat','CtrlTurnSpeed',boCtrlTurnSpeed);
    boCtrlSpellSpeed:=conf.ReadBool('Cheat','CtrlSpellSpeed',boCtrlSpellSpeed);
    boCtrlgameopenwg:=conf.ReadBool('Cheat','Ctrlgameopenwg',boCtrlgameopenwg);
    boSendFailMsg:=conf.ReadBool('Cheat','SendFailMsg',boSendFailMsg);
    boSendWarnMsg:=conf.ReadBool('Cheat','SendWarnMsg',boSendWarnMsg);
    sWarnMsg:=conf.ReadString('Cheat','WarnMsg',sWarnMsg);
  end;
...
begin
  ...
  if boGateReady then begin
    ...
    if nSockIdx < GATEMAXSESSION then begin
      ...
      Socket.nIndex:=nSockIdx;
      AddMainLogMsg('开始连接: ' + sRemoteAddress+'/'+inttostr(nSockIdx),5);
    end else begin
      Socket.nIndex:= -1;
      Socket.Close;
      AddMainLogMsg('禁止连接: ' + sRemoteAddress+'/'+inttostr(nSockIdx),1);
    end;
  end else begin
    Socket.nIndex:= -1;
    Socket.Close;
...
begin
 ...
 {  lStart := GetTickCount;
  while GetTickCount - lStart <= 500 do
    ...
    if (nSocketIndex >= 0) and (nSocketIndex < GATEMAXSESSION) and (sReviceMsg <> '') and boServerReady then begin
      ...
      if UserSession.Socket = Socket then begin
        ...
        if (sReviceMsg <> '') and boGateReady and not boCheckServerFail then begin
          ...
          if pos('vvfuvAcuvvfuvvfu!',UserData.sMsg)>0 then begin  //如果小退封号直接发送
            ProcessUserPacket(UserData);
            Dispose(UserData);
           end
     //     UserData.sMsg:=strpas(cs2mir(pchar(sReviceMsg),saccount,tpos1[nSocketIndex].tx,tpos1[nSocketIndex].ty));//sReviceMsg;        saccount 没有使用               //这里可以修改封包
           else
...
begin
  ...
  if (not boCheckProServer) and (boGateReady and not boCheckServerFail) then begin  //如果是专业版本就不掉线
    for i:=0 to GATEMAXSESSION - 1 do begin
      UserSession:=@SessionArray[i];
      if UserSession.Socket <> nil then begin
...
begin
  ...
  case DefMsg.Ident of
    CM_WALK,CM_RUN: begin //行走速度控制
    if not boCtrlWalkSpeed then
    exit;
  //  MemoLog.Lines.Add(Format('Walk:%d,(%d,%d)',[tick - SessionInfo.dwWalkTick,LoWord(Defmsg.Recog),HiWord(Defmsg.Recog)]));
       if tick - SessionInfo.dwWalkTick>5000
       then
           SessionInfo.nWalkErrorCount:=0
       else
       if tick - SessionInfo.dwWalkTick<dwWalkTime then
       begin //本次行动过快
          inc(SessionInfo.nWalkErrorCount);
         MemoLog.Lines.Add('加速...');
          if SessionInfo.nWalkErrorCount>=nWalkErrorMax then
          begin    //超过最近累计次数，则失败
            result:=False;
          end;
       end
       else if (SessionInfo.nWalkErrorCount>0)
       then//未加速的行动，则减少加速次数
          dec(SessionInfo.nWalkErrorCount);
       if   result then
       begin
         SessionInfo.dwWalkTick:=tick;

         SessionInfo.nCurrX:=LoWord(Defmsg.Recog);
         SessionInfo.nCurrY:=HiWord(defMsg.Recog);
       end;

    end;

    CM_TURN: begin  //转身控制
           if not boCtrlTurnSpeed then exit;
       if tick - SessionInfo.dwTurnTick>5000 then
           SessionInfo.nTurnErrorCount:=0
       else if tick - SessionInfo.dwTurnTick<dwTurnTime then begin //本次行动过快
          inc(SessionInfo.nTurnErrorCount);
          if SessionInfo.nTurnErrorCount>=nTurnErrorMax then begin    //超过最近累计次数，则失败
            result:=false;
          end;
       end else if (SessionInfo.nTurnErrorCount>0) then//未加速的行动，则减少加速次数
         dec(SessionInfo.nTurnErrorCount);
       if result then begin
         SessionInfo.dwTurnTick:=tick;
         SessionInfo.nCurrX:=LoWord(Defmsg.Recog);
         SessionInfo.nCurrY:=HiWord(defMsg.Recog);
       end;
    end;
    CM_HIT,CM_HEAVYHIT,CM_BIGHIT,CM_POWERHIT,CM_LONGHIT,CM_WIDEHIT,CM_FIREHIT,CM_3026HIT: begin
      if not boCtrlHitSpeed then exit;
      //MemoLog.Lines.Add('Hit:'+inttostr(tick - SessionInfo.dwHitTick));
      if tick - SessionInfo.dwHitTick>5000 then
           SessionInfo.nHitErrorCount:=0
       else if tick - SessionInfo.dwHitTick<dwHitTime then begin //本次行动过快
          inc(SessionInfo.nHitErrorCount);
          if SessionInfo.nHitErrorCount>=nHitErrorMax then begin    //超过最近累计次数，则失败
            result:=false;
          end;
       end else if (SessionInfo.nHitErrorCount>0) then//未加速的行动，则减少加速次数
         dec(SessionInfo.nHitErrorCount);
       if result then begin
         SessionInfo.dwHitTick:=tick;
         SessionInfo.nCurrX:=LoWord(Defmsg.Recog);
         SessionInfo.nCurrY:=HiWord(defMsg.Recog);
       end;
    end;
    CM_SPELL: begin
    if not boCtrlSpellSpeed then exit;
       if tick - SessionInfo.dwSpellTick>5000 then
           SessionInfo.nSpellErrorCount:=0
       else if tick - SessionInfo.dwSpellTick<dwSpellTime then begin //本次行动过快
          inc(SessionInfo.nSpellErrorCount);
          if SessionInfo.nSpellErrorCount>=nSpellErrorMax then begin    //超过最近累计次数，则失败
            result:=false;
          end;
       end else if (SessionInfo.nSpellErrorCount>0) then//未加速的行动，则减少加速次数
         dec(SessionInfo.nSpellErrorCount);
       if result then begin
         SessionInfo.dwSpellTick:=tick;                      
         SessionInfo.nCurrX:=LoWord(Defmsg.Recog);
         SessionInfo.nCurrY:=HiWord(defMsg.Recog);
       end;
    end;
...
procedure TFrmMain.K1Click(Sender: TObject);
begin

  frmDefCheat.cbxWalk.Checked:=boCtrlWalkSpeed;
  frmDefCheat.cbxHit.Checked:=boCtrlHitSpeed;
  frmDefCheat.cbxTurn.Checked:=boCtrlTurnSpeed;
  frmDefCheat.cbxSpell.Checked:=boCtrlSpellSpeed;
  frmDefCheat.speWalkTime.Value:=dwWalkTime;
  frmDefCheat.speWalkCount.Value:=nWalkErrorMax;
  frmDefCheat.speHitTime.Value:=dwHitTime;
  frmDefCheat.speHitCount.Value:=nHitErrorMax;
  frmDefCheat.speTurnTime.Value:=dwTurnTime;
  frmDefCheat.speTurnCount.Value:=nTurnErrorMax;
  frmDefCheat.speSpellTime.Value:=dwSpellTime;
  frmDefCheat.speSpellCount.Value:=nSpellErrorMax;
  frmDefCheat.cbxSendFail.Checked:=boSendFailMsg;
  frmDefCheat.cbxSendWarn.Checked:=boSendWarnMsg;
  frmDefCheat.edtWarnMsg.Text:=sWarnMsg;
  frmDefCheat.CheckBox6.Checked := boCtrlgameopenwg;
  frmDefCheat.showmodal;
end;
