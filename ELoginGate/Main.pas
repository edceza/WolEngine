...
uses
  Windows, Messages, SysUtils, Classes, Graphics, EDcode,
  Controls, Forms, Dialogs, StdCtrls, JSocket, WinSock, ExtCtrls, ComCtrls, Menus,
    IniFiles, GateShare;
const
...
begin
  ...
  begin
    ...
    begin                                                   //被攻击
      ...
      MainOutMessage('端口攻击,数据包过长: ' + sRemoteIPaddr, 1);
      exit;
    end;
...
begin
  ...
  if gettickcount - dwstartgate > 1000 * 60 * 60 * 3 then
  begin                                                     //1000*5
    dwstartgate := gettickcount;
    if ClientSocket.Active then
    begin
      ClientSocket.Socket.SendText('%G' +
        Inttostr(GatePort) +
        '$');
    end;
  end;
...
begin
  ...
  try
   ...
   //       sSocketMsg:=strpas(mir2cs(pchar(sSocketMsg),length(sSocketMsg),g_SessionArray[nSocketIndex].saccount));
...
begin
  ...
  begin
    UserSession := @g_SessionArray[nIndex];
    if (UserSession.Socket <> nil) and (UserSession.SocketHandle = nSocketHandle) then
    begin
      UserSession.Socket.Close;
      break;
    end;
...
begin
  ...
  nMaxMsgLen := Conf.ReadInteger(GateClass, 'MaxMsgLen', nMaxMsgLen);
...
begin
  ...
  frmIPaddrFilter.MaxMsgLen.Value := nMaxMsgLen;
  case BlockMethod of
...
begin
  ...
  begin
    ...
    begin
      Inc(IPaddr.nCount);
      if GetTickCount - IPaddr.dwIPCountTick1 < 1000 then
      begin
      ...
      if GetTickCount - IPaddr.dwIPCountTick2 < 3000 then
      begin
