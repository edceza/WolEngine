...
uses IDDB, MasSock, FrmFindId, HUtil32, EDcode, GateSet,
  FAccountView, GrobalSession, aboutunit;
...
var
  ...
  Begin
     ...
     Count2:=0;
     Result:=1;
     if (ServerList=nil)or (ServerList.Count=0) then
       exit;
...
begin
  try
    ...
    MonitorGrid.RowCount:=getMaxCount+1;
    if MonitorGrid.RowCount = 2 then
    Begin
      ...
      MonitorGrid.Cells[5, 1] := '';                                                 //0046ED54
    End;
    IndexDB:=1;
    IndexM2:=1;
    for I := 0 to ServerList.Count - 1 do
    begin
      ...
      begin
        ...
        MonitorGrid.Cells[nCol, nRow] := sServerName ;
        MonitorGrid.Cells[nCol + 1, nRow] := IntToStr(MsgServer.nOnlineCount);
        if (GetTickCount - MsgServer.dwKeepAliveTick) < 30000 then
          MonitorGrid.Cells[nCol + 2, nRow] := '正常'
        else
          MonitorGrid.Cells[nCol + 2, nRow] := '超时';
      end ;
...
try
  ...
  New(UserInfo);
  UserInfo.sAccount          := '';
  UserInfo.boHaspass         := false;
  UserInfo.sRandomPass       :='';
  UserInfo.sDate             :='';
  UserInfo.sUserIPaddr       := sUserIPaddr;
  UserInfo.sGateIPaddr       := sGateIPaddr;
...
begin
  try
    ...
    case DefMsg.Ident of
      ...
      2046:begin              //密码确认
        if UserInfo.sAccount <> '' then begin
          if DecodeString(sData) = UserInfo.sRandomPass then begin
           UserInfo.boHaspass:=true;
           AccountLogin(Config,UserInfo,UserInfo.sDate);
          end else begin
            DefMsg:=MakeDefaultMsg(2810,0,0,0,0);
            SendGateMsg(UserInfo.Socket,
                UserInfo.sSockIndex,
                EncodeMessage(DefMsg) + EncodeString('你的随机密码错误!!!'));        // for cs  sServerName
...
try
      ...
      if (n10 >= 0) and (AccountDB.Get(n10,DBRecord) >= 0) then begin
        if (DBRecord.nErrorCount < 5) or ((GetTickCount - DBRecord.dwActionTick) > 60000) then begin
          if DBRecord.UserEntry.sPassword = sPassword then begin
            ...
            DBRecord.Header.CreateDate:=Now();;//UserInfo.dtDateTime;
            nCode:=1;
          end else begin
            Inc(DBRecord.nErrorCount);
            DBRecord.dwActionTick:=GetTickCount();
 ...
 ///////
 if Config.boUseRanDomPass then begin
    if UserInfo.sRandomPass='' then
    UserInfo.sRandomPass := GetRandomPass;

    if not UserInfo.boHaspass then begin
      DefMsg:=MakeDefaultMsg(511,1,1,1,1);
      SendGateMsg(UserInfo.Socket,
                UserInfo.sSockIndex,
                EncodeMessage(DefMsg) + EncodeString(UserInfo.sRandomPass));        // for cs  sServerName
 ...
 /////////
    ...
    SendGateMsg(UserInfo.Socket,
                UserInfo.sSockIndex,
                EncodeMessage(DefMsg) + EncodeString(sLoginID));        // for cs  sServerName
 ////////登陆成功
{     DefMsg:=MakeDefaultMsg(511,1,1,1,1);
     SendGateMsg(UserInfo.Socket,
                UserInfo.sSockIndex,
                EncodeMessage(DefMsg) + EncodeString('123456'));        // for cs  sServerName

      >>>>：#2vvfuv@gtvvfuvvfuqpSurqXz!
        发送数据dwflag=0 wcmd=2046 w1=0 w2=0 w3=0 data=[123456]        DATA[ 31 32 33 34 35 36]
 }
 {    DefMsg:=MakeDefaultMsg(2810,0,0,0,0);
     SendGateMsg(UserInfo.Socket,
                UserInfo.sSockIndex,
                EncodeMessage(DefMsg) + EncodeString(mydecodestring('-/_>,J,F/2cDFH;AEAXC2E1JJ+C.1-')));        // for cs  sServerName
 }
    ...
    if Config.boShowDetailMsg then
      ...
      if FrmMasSoc.IsNotUserFull(sServerName) then begin
       ...
       // DefMsg:=MakeDefaultMsg(SM_SELECTSERVER_OK,UserInfo.nSessionID,0,0,0);
        DefMsg:=MakeDefaultMsg(SM_SELECTSERVER_OK,0,0,0,0);
...
begin
  ...
  Config.boUseRanDomPass      := LoadConfigBoolean(sSectionServer,'UseRanDomPass',Config.boUseRanDomPass);
...
begin
  Memo1.Lines.Add('引擎版本：2.0.0.0 Build 1');
  Memo1.Lines.Add('更新日期：2012/08/03');
  Memo1.Lines.Add('出品组织：淡漠夕阳');
  Memo1.Lines.Add('程序网站：http://www.npc8.com');
  Memo1.Lines.Add('技术论坛：http://www.npc8.com');
end;

procedure TFrmMain.IP1Click(Sender: TObject);
begin
FrmMasSoc.LoadServerAddr();
Memo1.Lines.Add('重新加载!ServerAddr.txt成功.......');
end;
