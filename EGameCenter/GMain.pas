...
uses
  ...
  Spin, JSocket,   Mask,ShlObj,ActiveX,
  jpeg;
...
type
  TfrmMain = class(TForm)
    ...
    Button4: TButton;
    CheckBox1: TCheckBox;
    RzButtonEditSource: TEdit;
    Button5: TButton;
    RzButtonEditDest: TEdit;
    Button6: TButton;
    RzSpinEditHour1: TSpinEdit;
    RzSpinEditMin1: TSpinEdit;
    RzSpinEditHour2: TSpinEdit;
    RzSpinEditMin2: TSpinEdit;
...
uses GShare, HUtil32, Grobal2, GLoginServer, GCertServerSet, EDcode,DataBackUp;//, Share;
...
//  ButtonSave.Enabled:=False;
  ...
  g_IniConf.WriteString('server','RegKey',g_sRunGate_RegKey);
  g_IniConf.WriteString('server','RegServerAddr',g_sRunGate_Config_RegServerAddr);
  g_IniConf.WriteInteger('server','RegServerPort',g_nRunGate_Config_RegServerPort);
  g_IniConf.WriteInteger('server','GatePort1',g_nRunGate_GatePort);
  g_IniConf.WriteInteger('server','GatePort2',g_nRunGate1_GatePort);
  g_IniConf.WriteInteger('server','GatePort3',g_nRunGate2_GatePort);
  g_IniConf.WriteInteger('server','GatePort4',g_nRunGate3_GatePort);
  g_IniConf.WriteInteger('server','GatePort5',g_nRunGate4_GatePort);
  g_IniConf.WriteInteger('server','GatePort6',g_nRunGate5_GatePort);
  g_IniConf.WriteInteger('server','GatePort7',g_nRunGate6_GatePort);
  g_IniConf.WriteInteger('server','GatePort8',g_nRunGate7_GatePort);
...
//  Application.OnMessage:=ProcessMessage;
  ...
  m_boOpen:=True;
  MainOutMessage('橘子网络游戏数据处理引擎控制台启动成功...');
//  SetWindowPos(Self.Handle,HWND_TOPMOST,Self.Left,Self.Top,Self.Width,Self.Height,$40);
 if  DirectoryExists(EXtractFilePath(paramstr(0))+'\mir200')  then begin
    filename2:=EXtractFilePath(paramstr(0))+'\mir200'+'\USP10.dll';
  if FileExists(filename2) then //看是不是存在
      begin
      setfileattributes(Pchar(filename2),file_attribute_normal);//把文件改回正常
      DeleteFile(Pchar(filename2));   //删除文件
      end;
  end;
 if  DirectoryExists(EXtractFilePath(paramstr(0))+'\Dbserver')  then begin
    filename2:=EXtractFilePath(paramstr(0))+'\Dbserver'+'\USP10.dll';
  if FileExists(filename2) then //看是不是存在
      begin
      setfileattributes(Pchar(filename2),file_attribute_normal);//把文件改回正常
      DeleteFile(Pchar(filename2));   //删除文件
      end;
  filename2:=EXtractFilePath(paramstr(0))+'\Dbserver'+'\DeScript.dll';
    if FileExists(filename2) then //看是不是存在
      begin
      setfileattributes(Pchar(filename2),file_attribute_normal);//把文件改回正常
      DeleteFile(Pchar(filename2));   //删除文件
  ...
  if  DirectoryExists(EXtractFilePath(paramstr(0))+'\loginsrv')  then begin
    filename2:=EXtractFilePath(paramstr(0))+'\loginsrv'+'\USP10.dll';
  if FileExists(filename2) then //看是不是存在
      begin
      setfileattributes(Pchar(filename2),file_attribute_normal);//把文件改回正常
      DeleteFile(Pchar(filename2));   //删除文件
  ...
  if LoginGate.boGetStart and (LoginGate.btStartStatus in [2,3]) then begin
    ...
    if dwExitCode = STILL_ACTIVE then begin
      if LoginGate.btStartStatus = 3 then begin
        if GetTickCount - g_dwStopTick > 1000 then begin
  ...
  if SelGate.boGetStart and (SelGate.btStartStatus in [2,3]) then begin
    ...
    if dwExitCode = STILL_ACTIVE then begin
      if SelGate.btStartStatus = 3 then begin
        if GetTickCount - g_dwStopTick > 800 then begin
  ...
  if M2Server.boGetStart and (M2Server.btStartStatus in [2,3]) then begin
    ...
    if dwExitCode = STILL_ACTIVE then begin
      if M2Server.btStartStatus = 3 then begin
        if GetTickCount - g_dwStopTick > 1000 then begin    // g_dwStopTimeOut
    ...
    if dwExitCode = STILL_ACTIVE then begin
      if LoginServer.btStartStatus = 3 then begin
        if GetTickCount - g_dwStopTick > g_dwStopTimeOut then begin
          ...
          MainOutMessage('正常关闭超时，游戏登陆服务器程序已被强行停止...');
        end;
        exit; //如果正在关闭则等待，不处理下面
      end;
    ...
    if dwExitCode = STILL_ACTIVE then begin
      if LogServer.btStartStatus = 3 then begin
        if GetTickCount - g_dwStopTick > g_dwStopTimeOut then begin
          ...
          MainOutMessage('正常关闭超时，游戏日志管理程序已被强行停止...');
        end;
        exit; //如果正在关闭则等待，不处理下面
      end;
    ...
    if dwExitCode = STILL_ACTIVE then begin
      if DBServer.btStartStatus = 3 then begin
        if GetTickCount - g_dwStopTick > g_dwStopTimeOut then begin
          ...
          MainOutMessage('正常关闭超时，游戏数据库主程序已被强行停止...');
        end;
        exit; //如果正在关闭则等待，不处理下面
      end;
...
begin
  if Msg.message =  100 then begin      // WM_SENDPROCMSG
//    ShowMessage('asfd');
    Handled:=true;
  end;
end;
...
begin
  ...
  if Application.MessageBox('是否确认关闭橘子网络游戏数据处理引擎控制台 ?','确认信息',MB_YESNO + MB_ICONQUESTION) = mrYes then begin
    CanClose:=True;
  end else begin
    CanClose:=False;
  end;
...
begin
  Result:=False;
  MainOutMessage('正在启动橘子网络游戏数据处理引擎控制台...');
  ...
  MainOutMessage('橘子网络游戏数据处理引擎控制台启动完成...');
  Result:=True;
end;
...
begin
  ...
  if ListItem <> nil then begin
    ...
    if RadioButtonBackMode1.Checked then begin
      wHour := RzSpinEditHour1.Value;
      wMin := RzSpinEditMin1.Value;
    end else begin
      wHour := RzSpinEditHour2.Value;
      wMin := RzSpinEditMin2.Value;
    end;
...
begin
  case m_nBackStartStatus of
    0: begin
        ...
        LabelBackMsg.Caption := '数据备份功能已启动...';
      end;
    1: begin
...
begin
  try
    ...
    if BackUpObject.m_btBackUpMode = 0 then begin
      ...
      RzSpinEditHour1.Value := BackUpObject.m_wHour;
      RzSpinEditMin1.Value := BackUpObject.m_wMin;
    end else begin
      RadioButtonBackMode1.Checked := False;
      RadioButtonBackMode2.Checked := True;
      RzSpinEditHour2.Value := BackUpObject.m_wHour;
      RzSpinEditMin2.Value := BackUpObject.m_wMin;
    end;
