...
const
  MAXDEFCOLOR                                          = 16;
  ColorNames                                             : array[1..MAXDEFCOLOR] of
    TyNameTable                                          = (
  ...
  MAXLISTMARKER                                        = 3;
  LiMarkerNames                                          : array[1..MAXLISTMARKER] of
    TyNameTable                                          = (
    (Name: 'DISC'; varl: 0),
    (Name: 'CIRCLE'; varl: 1),
    (Name: 'SQUARE'; varl: 2)
  ...
  MAXPREDEFINE                                         = 3;
  PreDefineNames                                         : array[1..MAXPREDEFINE] of
    TyNameTable                                          = (
    (Name: 'LEFT'; varl: 0),
    (Name: 'RIGHT'; varl: 1),
    (Name: 'CENTER'; varl: 2)
...
{*}
function ArrestStringEx(Source, SearchAfter, ArrestBefore: string; var ArrestStr:
  string): string;
function CaptureString(source: string; var rdstr: string): string;
procedure ClearWindow(aCanvas: TCanvas; aLeft, aTop, aRight, aBottom: Longint; aColor:
  TColor);
...
{*}
function GetValidStr3(Str: string; var Dest: string; const Divider: array of Char):
  string;
function GetValidStr4(Str: string; var Dest: string; const Divider: array of Char):
  string;
function GetValidStrVal(Str: string; var Dest: string; const Divider: array of Char):
  string;
function GetValidStrCap(Str: string; var Dest: string; const Divider: array of Char):
  string;
...
function formatstr(sMsg, sText: string): string;
...
function Str_ToWord(Str: string; def: Longint): LongWord;
...
function GetLow(value: Byte): Byte;
function GetHigh(value: Byte): Byte;
implementation
...
function Replacestr(sMsg, sStr, sText: string): string;     //0049ADB8
var
  n10                                                  : Integer;
  s14, s18                                             : string;
begin
...
var
  i                                                    : Integer;
  s1                                                   : array[0..9] of string;
begin
  for I := 0 to 9 do
  begin
    sText := GetValidStr3(sText, s1[i], ['|']);
    if sText = '' then break;

  end;
  sMsg := AnsiReplaceText(sMsg, '#32', ' ');

  Result := Format(sMsg, [s1[0], s1[1], s1[2], s1[3], s1[4], s1[5], s1[6], s1[7], s1[8],
    s1[9]]);
end;

function Replaceallstr(sMsg, sStr, sText: string): string;  //0049ADB8
var
  a                                                    : TReplaceFlags;
begin
  include(a, rfReplaceAll);
  include(a, rfIgnoreCase);
  Result := stringReplace(sMsg, sStr, sText, a);
end;
...
begin
  ...
  Dec(YearMax, YearMin);
  YearMin := 0;
  Result := (YearMax * 12 * 30 + MonthMax * 30 + DayMax) - (YearMin * 12 * 30 + MonthMin
    * 30 + DayMin);
end;
