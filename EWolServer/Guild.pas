...
//    MatchPoint   :Integer;
  ...
  private
    ...
    procedure SetBuildPoint(nPoint: Integer);
     procedure SetEXP(nPoint: Integer);
     procedure Setlevel(nPoint: Integer);
    ...
    procedure SetMemberMaxCount(nPoint: Integer);
  ...
  public
    ...
    function LoadGuildPoint(sGuildFileName: string): string; //倚天扩展。
    ...
    property nEXP: Integer read m_nEXP write SetEXP;
    property nLevel: Integer read m_nLevel write SetLevel;
    ...
    property nMemberMaxCount: Integer read m_nMemberMaxCount write SetMemberMaxCount;    
  end;
  TGuildManager = class
    GuildList: TList; //0x4
  private
...
begin
  ...
  m_nMemberMaxCount:=g_Config.nGuildMembersMax;
...
function TGuild.LoadGuildPoint(sGuildFileName: string): string;
var
  GuildPoint: string;
  Guild_Config: TiniFile;
begin
  Guild_Config := TiniFile.Create(g_Config.sGuildDir + sGuildFileName + '.ini');
  GuildPoint := Guild_Config.ReadString('Guild', 'BuildPoint', GuildPoint);
  MainOutMessage(g_Config.sGuildDir + sGuildFileName + '.ini' + '本次数据：' + GuildPoint);
  Guild_Config.Free;
  Result := GuildPoint;
end;
...
begin
  ...
  m_nExp := m_Config.ReadInteger('Guild', 'GuildExp', m_nBuildPoint);
  m_nLevel := m_Config.ReadInteger('Guild', 'GuildGrade', m_nBuildPoint);
  ...
  m_nMemberMaxCount := m_Config.ReadInteger('Guild', 'GuildMemberMaxLimit', m_nMemberMaxCount);
  Result := True;
end;
...
begin
  ...
  m_Config.WriteInteger('Guild', 'GuildExp', m_nExp);
  m_Config.WriteInteger('Guild', 'GuildGrade', m_nLevel);
  ...
  m_Config.WriteInteger('Guild', 'GuildMemberMaxLimit', m_nMemberMaxCount);
...
begin
  Result := False;
  GuildRank18 := nil;
  if count>=m_nMemberMaxCount then exit;
  for I := 0 to m_RankList.Count - 1 do
  begin
    GuildRank := m_RankList.Items[I];
...
function TGuild.UpdateRank(sRankData: string): Integer; //00499140
  ...
  function findRand(RankList: TList;nRankNo:Integer):pTGuildRank;
  var
  I: Integer;
  GuildRank: pTGuildRank;
  begin
  ...
  function CheckRand(RankList: TList;nRankNo:Integer;sRandName:String):Boolean;
  var
  I: Integer;
  GuildRank: pTGuildRank;
  begin
    ...
    begin
      GuildRank := RankList.Items[I];
      if (GuildRank.sRankName=sRandName)and(GuildRank.nRankNo<>nRankNo ) then
      begin
        Result:=False;
        Break;
      end;
  ...
  function DelMember(RankList: TList;sHumName: string): Boolean; //00498DCC
var
  I, II: Integer;
  GuildRank: pTGuildRank;
begin
...
var
  ...
  GuildRankList: TList;
  GuildRank: pTGuildRank;
  NewGuildRank,NewGuildRank1: pTGuildRank;
  sRankInfo: string;
  sRankNo: string;
  sRankName: string;
  sMemberName: string;
 ...
 // MainOutMessage(sRankData);
  ...
  begin
    ...
    begin //取得职称的名称
      ...
      if (GuildRank <> nil)and(sRankName<>'') then
      begin
    ...
    begin //将成员名称加入职称表里
      ...
      if (sMemberName <> '')and IsMember(sMemberName) then
      Begin
        GuildRank.MemberList.Add(sMemberName);
        DelMember(m_RankList,sMemberName);
      End
      else
      Begin
        Result:=-6;  //不是行会的人
  ...
  for ii := 0 to GuildRankList.Count - 1 do
  begin
    GuildRank := GuildRankList.Items[II];
    if CheckRand(m_RankList,GuildRank.nRankNo,GuildRank.sRankName) then
    begin
      NewGuildRank :=findRand(m_RankList,GuildRank.nRankNo);
      if NewGuildRank<>nil then
      begin
        if (NewGuildRank.nRankNo=1) then
        begin
          n28:= NewGuildRank.MemberList.Count+GuildRank.MemberList.Count;
          if n28> 2 then  //  掌门超过2个
          begin
            Result:=-4;
            Exit;
          end;
        ...
        NewGuildRank.sRankName:=GuildRank.sRankName;
        NewGuildRank.MemberList.AddStrings(GuildRank.MemberList);
      end
      else
      Begin
          New(NewGuildRank1);
          NewGuildRank1.nRankNo := GuildRank.nRankNo;
          NewGuildRank1.sRankName := GuildRank.sRankName;
          NewGuildRank1.MemberList := TStringList.Create;
          NewGuildRank1.MemberList.AddStrings(GuildRank.MemberList);
          m_RankList.Add(NewGuildRank1) ;
      End;
    end
    else
      Result:=-7;
...
//  begin
//    boCheckChange := True;
//    for I :=  m_RankList.Count - 1 downto 0  do
//    begin
//      NewGuildRank := m_RankList.Items[I];
//      boCheckChange:=False;
//      for ii := 0 to GuildRankList.Count - 1 do
//      begin
//        GuildRank := GuildRankList.Items[II];
//         if (GuildRank.nRankNo = NewGuildRank.nRankNo) then
//         Begin
//              boCheckChange:=True;
//             Break;
//         End;
...
//           Begin
//                New(NewGuildRank1);
//                NewGuildRank1.nRankNo := NewGuildRank.nRankNo;
//                NewGuildRank1.sRankName := NewGuildRank.sRankName;
//                NewGuildRank1.MemberList := TStringList.Create;
//                NewGuildRank1.MemberList.Text:=NewGuildRank.MemberList.Text;
//               GuildRankList.Insert(ii,NewGuildRank1);
//             boCheckChange:=True;
//           End;
//           if not boCheckChange then
//           Begin
...
//                NewGuildRank1.MemberList := TStringList.Create;
//                NewGuildRank1.MemberList.Text:=NewGuildRank.MemberList.Text;
//             GuildRankList.Add(NewGuildRank1);
//           End;
//           break;
//         end;
//      End;
...
procedure TGuild.SetEXP(nPoint: Integer);
begin
  m_nEXP := nPoint;
  boChanged := True;
end;
procedure TGuild.SetLevel(nPoint: Integer);
begin
  m_nLevel := nPoint;
  boChanged := True;
end;
procedure TGuild.SetMemberMaxCount(nPoint: Integer);
begin
  m_nMemberMaxCount := nPoint;
  boChanged := True;
end;
