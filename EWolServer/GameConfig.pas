...
uses
  ...
  Dialogs, ComCtrls, StdCtrls, Spin, Grids, ExtCtrls,IniFiles;
...
type
  ...
  TfrmGameConfig = class(TForm)
    ...
    Label15: TLabel;
    SafeZoneLightType: TSpinEdit;
    BoUseSafeZoneLight: TCheckBox;
    GroupBox75: TGroupBox;
    hitshow: TCheckBox;
    safehite: TCheckBox;
    showpresent: TCheckBox;
    goldaddinbag: TCheckBox;
    SpinEditDropItemRage: TSpinEdit;
    Label145: TLabel;
    CheckBoxOffLineSuper: TCheckBox;
    CheckBoxUsePaihang: TCheckBox;
    CheckBoxSafeOffLine: TCheckBox;
    GroupBox76: TGroupBox;
    Label146: TLabel;
    EditGuildMembersMax: TSpinEdit;
    GroupBox77: TGroupBox;
    Label147: TLabel;
    Label148: TLabel;
    LabelYellowMsgFColor: TLabel;
    LabelYellowMsgBColor: TLabel;
    EditYellowMsgFColor: TSpinEdit;
    EditYellowMsgBColor: TSpinEdit;
    Checkboxdisablesell: TCheckBox;
    CheckboxdisablePetsell: TCheckBox;
    CheckBoxLingzu: TCheckBox;
    ...
    procedure BoUseSafeZoneLightClick(Sender: TObject);
    procedure SafeZoneLightTypeChange(Sender: TObject);
    procedure hitshowClick(Sender: TObject);
    procedure safehiteClick(Sender: TObject);
    procedure goldaddinbagClick(Sender: TObject);
    procedure SpinEditDropItemRageChange(Sender: TObject);
    procedure showpresentClick(Sender: TObject);
    procedure CheckBoxOffLineSuperClick(Sender: TObject);
    procedure CheckBoxUsePaihangClick(Sender: TObject);
    procedure CheckBoxSafeOffLineClick(Sender: TObject);
    procedure EditGuildMembersMaxChange(Sender: TObject);
    procedure EditYellowMsgFColorChange(Sender: TObject);
    procedure EditYellowMsgBColorChange(Sender: TObject);
    procedure CheckboxdisablesellClick(Sender: TObject);
    procedure CheckboxdisablePetsellClick(Sender: TObject);
    procedure CheckBoxLingzuClick(Sender: TObject);
  private
    boOpened: Boolean;
    boModValued: Boolean;
    procedure ModValue();
...
begin
  ...
  EditSoftVersionDate.Hint := '客户端版本日期设置，此参数默认为 20120728，此版本日期必须与客户端相匹配，否则在进入游戏时将提示版本不正确';
...
*)
  ...
  BoUseSafeZoneLight.Checked := g_Config.boUseSafeZoneLight;
  SafeZoneLightType.Value := g_Config.nUseSafeZoneLightType;
  ...
  EditGuildMembersMax.Value := g_Config.nGuildMembersMax;
  ...
  EditYellowMsgFColor.Value := g_Config.btYellowMsgFColor;
  EditYellowMsgBColor.Value := g_Config.btYellowMsgBColor;
  ...
  Checkboxdisablesell.Checked :=g_Config.boDisableSell;
  CheckboxdisablePetsell.Checked :=g_Config.boDisablePetSell ;  
...
procedure TfrmGameConfig.CheckboxdisablesellClick(Sender: TObject);
begin
  if not boOpened then
    exit;
  g_Config.boDisableSell := CheckBoxDisableSell.Checked;
  ModValue();
end;
...
procedure TfrmGameConfig.CheckBoxLingzuClick(Sender: TObject);
begin
  if not boOpened then
    exit;
  g_Config.boUseLingzu := CheckBoxLingzu.Checked;
  ModValue();
end;
...
procedure TfrmGameConfig.EditYellowMsgBColorChange(Sender: TObject);
var
  btColor: Byte;
begin
  btColor := EditYellowMsgBColor.Value;
  LabelYellowMsgBColor.Color := GetRGB(btColor);
  if not boOpened then
    exit;
  g_Config.btYellowMsgBColor := btColor;
  ModValue();
end;
procedure TfrmGameConfig.EditYellowMsgFColorChange(Sender: TObject);
var
  btColor: Byte;
begin
  btColor := EditYellowMsgFColor.Value;
  LabelYellowMsgFColor.Color := GetRGB(btColor);
  if not boOpened then
    exit;
  g_Config.btYellowMsgFColor := btColor;
  ModValue();
end;
...
procedure TfrmGameConfig.EditGuildMembersMaxChange(Sender: TObject);
begin
  if not boOpened then
    exit;
  g_Config.nGuildMembersMax := EditGuildMembersMax.Value;
  ModValue();
end;
...
begin
  ...
  Config.WriteInteger('Setup', 'YellowMsgFColor', g_Config.btYellowMsgFColor);
  Config.WriteInteger('Setup', 'YellowMsgBColor', g_Config.btYellowMsgBColor);
...
begin
  ...
  hitshow.checked := boolean(g_Config.ShowHitHp);
  safehite.Checked := boolean(g_Config.SafeAreaHint);
  showpresent.Checked := g_Config.ShowHpPresent;
  CheckBoxOffLineSuper.Checked := g_Config.OffLineSuper;

  CheckBoxUsePaihang.Checked := g_Config.boUsePaihang;
  CheckBoxLingzu.Checked :=g_Config.boUseLingzu;



  goldaddinbag.Checked := boolean(g_Config.GoldAddinBag);
  SpinEditDropItemRage.Value := g_Config.nDropItemRage;
end;
...
procedure TfrmGameConfig.BoUseSafeZoneLightClick(Sender: TObject);
begin
  if not boOpened then
    exit;
  g_Config.boUseSafeZoneLight := BoUseSafeZoneLight.Checked;
  ModValue();
end;

procedure TfrmGameConfig.SafeZoneLightTypeChange(Sender: TObject);
begin
  if not boOpened then
    exit;
  g_Config.nUseSafeZoneLightType := SafeZoneLightType.Value;
  ModValue();
end;

procedure TfrmGameConfig.hitshowClick(Sender: TObject);
begin
  if not boOpened then
    exit;
  g_Config.ShowHitHp := integer(hitshow.Checked);
  ModValue();
end;

procedure TfrmGameConfig.safehiteClick(Sender: TObject);
begin
  if not boOpened then
    exit;
  g_Config.SafeAreaHint := integer(safehite.Checked);
  ModValue();
end;

procedure TfrmGameConfig.goldaddinbagClick(Sender: TObject);
begin
  if not boOpened then
    exit;
  g_Config.GoldAddinBag := integer(goldaddinbag.Checked);
  ModValue();
end;

procedure TfrmGameConfig.SpinEditDropItemRageChange(Sender: TObject);
begin
  if not boOpened then
    exit;
  g_Config.nDropItemRage := SpinEditDropItemRage.Value;
  ModValue();
end;

procedure TfrmGameConfig.showpresentClick(Sender: TObject);
begin
  if not boOpened then
    exit;
  g_Config.ShowHpPresent := showpresent.Checked;
  ModValue();
end;

procedure TfrmGameConfig.CheckBoxOffLineSuperClick(Sender: TObject);
begin
  if not boOpened then
    exit;
  g_Config.OffLineSuper := CheckBoxOffLineSuper.Checked;
  ModValue();
end;

procedure TfrmGameConfig.CheckboxdisablePetsellClick(Sender: TObject);
begin
  if not boOpened then
    exit;
  g_Config.boDisablePetSell := CheckBoxDisablePetSell.Checked;
  ModValue();
end;

procedure TfrmGameConfig.CheckBoxUsePaihangClick(Sender: TObject);

begin
  if not boOpened then
    exit;
  g_Config.boUsePaihang := CheckBoxUsePaihang.Checked;
  ModValue();
end;
