...
type
  ...
  public
    ...
    procedure AddToLoadRcdList(sAccount, sChrName, sIPaddr: string; boFlag: Boolean; nSessionID: integer; nPayMent, nPayMode, nSoftVersionDate, nSocket, nGSocketIdx, nGateIdx: Integer;boys:Boolean);
    procedure AddLoadLocalIp;
...
var
  ...
  boReTryLoadDB, boProceSavrcd, boNewYsFault: Boolean;
begin
  ChangeGoldList := nil;
  ...
  begin
    ...
    boProceSavrcd := false;
    boNewYsFault := false;
    if SaveRcd.boNewYs and SaveHumRcdToDB(SaveRcd.sAccount, SaveRcd.sChrName, SaveRcd.nSessionID,'1', SaveRcd.HumanRcd) then
    begin
      boProceSavrcd := true;
    end
    else
    ...
    if (not SaveRcd.boNewYs) and SaveHumRcdToDB(SaveRcd.sAccount, SaveRcd.sChrName, SaveRcd.nSessionID,'0', SaveRcd.HumanRcd) or (SaveRcd.nReTryCount > 50) then
    begin
      boProceSavrcd := true;
    end;

    if boProceSavrcd or boNewYsFault then
    begin
      if (SaveRcd.PlayObject <> nil) then
      begin
        if SaveRcd.boNewYs then
        begin
          if (not boNewYsFault) then
          begin
            TPlayObject(SaveRcd.PlayObject).sYsname := SaveRcd.sChrName;
            TPlayObject(SaveRcd.PlayObject).nyssex := SaveRcd.HumanRcd.data.btsex;
          
          end;
          TPlayObject(SaveRcd.PlayObject).m_boCheckNewYs := true;
        end
        else
          if boProceSavrcd then
    ...
    begin
     // if SaveRcd.boNewYs then begin
     //  TPlayObject(SaveRcd.PlayObject).m_boCheckNewYs:=true;

     // end;
      Inc(SaveRcd.nReTryCount);
    end; //  if boProceSavrcd then begin
  end; //004B4FDA for I:=0 to m_SaveRcdTempList.Count -1
  m_SaveRcdTempList.Clear;
 
  for I := 0 to m_LoadRcdTempList.Count - 1 do
  begin
    LoadDBInfo := m_LoadRcdTempList.Items[I];
    if LoadDBInfo.boLoadLocalIp then
    begin
      DBLocalIp;
      boReTryLoadDB := false;
    end
    ...
    begin
      ...
      begin
       ...
       //  MainOutMessage('xjs:if not LoadHumFromDB(LoadDBInfo,boReTryLoadDB) then RunSocket.CloseUser');
...
begin
  ...
  EnterCriticalSection(m_UserCriticalSection);
  try
    if m_SaveRcdList.Count >= 2000 then
    begin
      Result := True;
    end;
  finally
...
procedure TFrontEngine.AddLoadLocalIp;
var
  LoadRcdInfo: pTLoadDBInfo;
begin
  New(LoadRcdInfo);
  LoadRcdInfo.boLoadLocalIp := true;
  EnterCriticalSection(m_UserCriticalSection);
  try
...
procedure TFrontEngine.AddToLoadRcdList(sAccount, sChrName, sIPaddr: string;
  boFlag: Boolean; nSessionID, nPayMent, nPayMode, nSoftVersionDate, nSocket, nGSocketIdx, nGateIdx: Integer;boys:Boolean);
//004B46A0
var
  LoadRcdInfo: pTLoadDBInfo;
begin
  ...
  LoadRcdInfo.boLoadLocalIp := false;
  LoadRcdInfo.boYs:=boys;
  EnterCriticalSection(m_UserCriticalSection);
  try
    m_LoadRcdList.Add(LoadRcdInfo);
  finally
...
begin
  ...
  hum := nil;
  hum := UserEngine.GetPlayObjectEx(LoadUser.sCharName);

  if (hum <> nil) then
  begin
   // UserEngine.KickPlayObjectEx(LoadUser.sCharName);
    UserEngine.SaveHumanRcd(hum);

    hum.m_boDeathRcdSaved := true;
    hum.makeghost;
  //  boReTry:=True; //反回TRUE,则重新加入队列
    exit;
  end;

  if not LoadHumRcdFromDB(LoadUser.sAccount, LoadUser.sCharName, LoadUser.sIPaddr, LoadUser.boYs, HumanRcd, LoadUser.nSessionID) then
  begin
...
begin
  Result := False;
  if LoadHumRcdFromDB('1', GoldChangeInfo.sGetGoldUser, '1',False, HumanRcd, 1) then
  begin
    if ((HumanRcd.Data.nGold + GoldChangeInfo.nGold) > 0) and ((HumanRcd.Data.nGold + GoldChangeInfo.nGold) < 2000000000) then
    begin
      Inc(HumanRcd.Data.nGold, GoldChangeInfo.nGold);
      if SaveHumRcdToDB('1', GoldChangeInfo.sGetGoldUser, 1, '0',HumanRcd) then
      begin
