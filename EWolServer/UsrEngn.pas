  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          Merchant.LoadNPCScript();                         //加载NPC 脚本
              ...
              begin
                ...
                begin                                       //说明是元神
                  ...
                  begin
                    ...
                    PlayObject.SendDefMessage(38291, integer(PlayObject), 4, 0, 0, '');  //背包的大小
...
{$IFEND}
        ...
        begin //如果当前 刷怪列表中的 怪物死了。
          if (GetTickCount - Monster.m_dwGhostTick) > 1 * 60 * 1000 then
          begin
            MonGen.CertList.Delete(nProcessPosition);
            //  MainOutMessage(format('死亡怪物释放名称:%s/位置:%d',[Monster.m_sCharName,nProcessPosition]));
...
begin
  ...
  case nMonRace of
    ...
    99:
      begin
        Cert := TNIMOKEMonster.Create;                      //逆魔怪物 的壳
    ...
    112: Cert := TArcherGuard.Create;                       //弓箭手
    ...
    122:
      begin
        Cert := TNIMOKEMonster.Create;                      //  //逆魔卫士
        TNIMOKEMonster(Cert).pm_wAppr := 87;
      end;
    123:
      begin
        Cert := TNIMOKEMonster.Create;                      // //逆魔雕像
        TNIMOKEMonster(Cert).pm_wAppr := 86;
      end;
    124:
      begin
        Cert := TNIMOAXEMonster.Create;                     //逆魔弓箭手
        TNIMOAXEMonster(Cert).pm_wAppr := 85;
      end;
    125:
      begin
        Cert := TNIMOKEMonster.Create;                      //  //逆魔侍卫
        TNIMOKEMonster(Cert).pm_wAppr := 128;
      end;
    126, 108:
      begin
        Cert := TScultureKingMonster.Create;                //逆魔
        TScultureKingMonster(Cert).pm_wAppr := 88;
        //  TMonster(Cert).m_btusefire:= 5
      end;

    200: Cert := TElectronicScolpionMon.Create;
    201, 129: Cert := Ttiexueking.Create;                   //铁血魔王
    16:
      begin
        Cert := Taiman.Create;
    ...
    202:
      begin //走动的全屏幕攻击   通天教主类型
        Cert := Tbigheatmovemonster.Create;
      end;
    203: Cert := Tbigheatmonsteraxe.Create;                 // 投石巨魔 全屏幕攻击 +麻痹
    204: Cert := TSeeBeeQueen.Create;                       //寒荒海兽
    205: Cert := TAXEHIDEMonster.Create;                    //魔眼
    206:
      begin
    ...
    207:                                                    //地火兽骑将
      begin
        ...
        TMonster1(Cert).m_btusefire := 5                    //火墙效果
      end;
    208:
      begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Cert.m_sNewName := IntToStr(Mongen.nLevel) + '级' + Mongen.sMonNewName;
        end;
      end;
    end;
  except
...
begin
  ...
  // PlayObject.m_nGold              := HumData.m_nGold;
 {
  ...
  // PlayObject.m_Abil.dw1AC         := HumData.Abil.dw1AC; //声望值
  ...
  //  PlayObject.m_wStatusTimeArr     := HumData.m_wStatusTimeArr;     //hint  人物状态保存读取
  ...
  // PlayObject.m_btCreditPoint      := HumData.m_btCreditPoint;   //声望
  // PlayObject.m_btReLevel          := HumData.m_btReLevel;
  PlayObject.m_bMaxBagitem := HumData.m_bMaxBagitem;        //最大包裹容量
  //  PlayObject.m_sSytleName         := HumData.m_sSytleName;     //个性化签名
  ...
  MagicInfo := UserEngine.FindMagic(2);                     //添加一个治疗术
...
begin
  ...
  PlayObject.m_wStatusTimeArr := HumData.wStatusTimeArr;    //hint  人物状态保存读取
  ...
  PlayObject.m_btCreditPoint := HumData.btCreditPoint;      //声望
...
begin
  ...
  begin
    PlayObject := TPlayObject(m_PlayObjectList.Objects[i]);
    if (not PlayObject.m_boGhost) and (PlayObject.sYsnameMaster = '') then //如果不是元神
...
{$IFEND}
  //     TClassProc(PlugProcArray[nEngRemoteRun].nProcAddr)(Self);    //有问题
{$IF DEBUG = 0}
  asm
    jz @@Start
    jnz @@Start
