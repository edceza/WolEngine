...
//  sDBHeaderDesc      :String = 'legend of mir database file 1999/1';
 ...
 ID_sRemoteAddress: string;
  ...
  g_boAutoBackup : Boolean;
  g_boBackUpDay  : Boolean;
  g_nBackupDay   : Integer;
  g_nBackupime   : TDateTime; 
  g_sBackupPath  : String;
implementation
...
begin
  ...
  if Conf <> nil then begin
    ...
    g_boAutoBackup:=Conf.ReadBool('BackUp','Enable',false);
    g_boBackUpDay:=Conf.ReadBool('BackUp','BackUpDay',true);
    g_nBackupDay:=Conf.ReadInteger('Backup','Day',1);
    g_nBackupime:=Conf.ReadDateTime('Backup','Time',StrToDateTime('00:00:00'));
    g_sBackupPath:=Conf.ReadString('Backup','Path','.\FDB\backup\');
    Conf.Free;
  end;
  LoadIPTable();
  LoadGateID();
