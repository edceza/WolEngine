...
interface
uses
  Windows,Classes,SysUtils,Forms,MudUtil,Grobal2,Controls,localDb;
ResourceString
  sDBHeaderDesc     = '橘子网络游戏数据库文件 2010/01/10';
  sDBIdxHeaderDesc  = '橘子网络游戏数据库索引文件 2010/01/10';
...
type
  ...
  pTseachItems =^TseachItems;
  TItems =record
    charnameid:integer;
    itemmakeindex:integer;
    iteminbagtype:integer;
    iteminbagpos:integer;
  end;
  pTItems=^TItems;
  TIdxHeader = record
  ...
  public
    ...
    procedure Rebuild();
    function CleardupItem():integer;
    procedure ProcecleardupItem(dupItemlist:Tlist) ;
    procedure FindItemByItemName(sItemName:string;var charItmeslist:Tlist);
    procedure FindItemByIndex(MakeIndex:integer;var charItmeslist:Tlist);
...
begin
if pTItems(Item1)^.itemmakeindex > pTItems(Item2)^.itemmakeindex then
  Result:=1   //返回大于0表示 Item1 是大于 Item2 的
 else if pTItems(Item1)^.itemmakeindex < pTItems(Item2)^.itemmakeindex then
...
Procedure ProcefinddupItem(itemlist:Tlist;var dupItemlist:Tlist);
var I,II:Integer;
    ppTItems,oldppTItems:pTItems;

begin
    oldppTItems:=pTItems( itemlist.Items[0]);
    i:=1 ;
    while i<  itemlist.Count do begin
        if pTItems(itemlist.Items[i]).itemmakeindex = oldppTItems.itemmakeindex then begin
          dupItemlist.Add(pTItems(itemlist.Items[i]));
          dupItemlist.Add(oldppTItems);
          if i+1 <= itemlist.Count-1 then
             oldppTItems:=pTItems( itemlist.Items[i+1]);
          inc(i,2);
        end else begin
          oldppTItems:=pTItems( itemlist.Items[i]);
          inc(i,1);
        end;
        //if =  makeindex
   end; //while
...
Procedure TFileDB.ProcecleardupItem( dupItemlist:Tlist);
var I,II:Integer;
    ppTItems,oldppTItems:pTItems;
    HumanRCD :THumDataInfo;
    charid:integer;
    bo15:boolean;
    iteminbagtype,iteminbagpos,itemmakeindex:integer;
    UserItem:pTUserItem;
    SaveList:TStringList;
    sFileName,sMsg:String;
begin
    SaveList:=TStringList.Create;
    sFileName:='清除复制品列表.txt';
    if FileExists(sFileName) then begin
      SaveList.LoadFromFile(sFileName);
    end;
  try
   if HumDataDB.Open then begin
    for i:=0 to dupItemlist.Count-1 do begin
      charid:= pTItems(dupItemlist.Items[i]).charnameid ;
      iteminbagtype :=pTItems(dupItemlist.Items[i]).iteminbagtype;
      iteminbagpos  :=pTItems(dupItemlist.Items[i]).iteminbagpos;
      itemmakeindex :=pTItems(dupItemlist.Items[i]).itemmakeindex;

      if (charid >= 0) and HumDataDB.GetRecord(charid,HumanRCD)  then bo15:=True;
      if bo15 then begin
         case iteminbagtype of
         0:  begin
                   UserItem:=@HumanRCD.Data.HumItems[iteminbagpos];
                   if UserItem.MakeIndex= itemmakeindex then begin
                      sMsg:=HumanRCD.Data.sChrName + #9 + GetStdItemName(UserItem.wIndex) + #9 + IntToStr(UserItem.MakeIndex);

                      UserItem.MakeIndex:=0;
                      UserItem.wIndex:=0;
                    HumDataDB.UpdateRecord(charid,HumanRCD,False);

                   end;
             end;
         2:  begin
                   UserItem:=@HumanRCD.Data.StorageItems[iteminbagpos];
                   if UserItem.MakeIndex= itemmakeindex then begin
                     sMsg:=HumanRCD.Data.sChrName + #9 + GetStdItemName(UserItem.wIndex) + #9 + IntToStr(UserItem.MakeIndex);
         ...
         3: begin
                   UserItem:=@HumanRCD.Data.BagItems[iteminbagpos];
                   if UserItem.MakeIndex= itemmakeindex then begin
                     sMsg:=HumanRCD.Data.sChrName + #9 + GetStdItemName(UserItem.wIndex) + #9 + IntToStr(UserItem.MakeIndex);
         ...
         4: begin
                   UserItem:=@HumanRCD.Data.monStorageItems[iteminbagpos];
                   if UserItem.MakeIndex= itemmakeindex then begin
                     sMsg:=HumanRCD.Data.sChrName + #9 + GetStdItemName(UserItem.wIndex) + #9 + IntToStr(UserItem.MakeIndex);
   ...
   ////
        ...
        //  sMsg:=HumanRCD.Data.sChrName + #9 + IntToStr(UserItem.wIndex) + #9 + IntToStr(UserItem.MakeIndex);
...
procedure TFileDB.FindItemByIndex(MakeIndex:integer;var charItmeslist:Tlist);
 var  I,ii:integer;
      fHUMStream:tStream;
      humcount:integer;
      DBHeader:TDBHeader;
      ...
      ppTItems:pTseachItems;
      UserItem:pTUserItem;

begin
       fHUMStream := TFileStream.Create(m_sDBFileName,fmShareDenyNone	 ); // fmCreate
     //  charItmeslist:=Tlist.Create;
     //  dupItemlist:=Tlist.Create;
     //  result:=0;
      // T:=GetTickCount;
    if fHUMStream <> nil  then   begin
     // FileRead(humFileHandle,DBHeader,SizeOf(TDBHeader));
        fHUMStream.ReadBuffer(DBHeader,SizeOf(TDBHeader));
        humcount:=DBHeader.nHumCount  ;
       
        for i:=0 to  humcount-1 do begin
         fHUMStream.Position:= i * SizeOf(THumDataInfo) + SizeOf(TDBHeader);
         if  fHUMStream.Read(ChrRecord,SizeOf(THumDataInfo)) <> SizeOf(THumDataInfo) then break;
          if ChrRecord.data.sChrName<>'' then begin
            for ii:=Low(ChrRecord.Data.HumItems) to High(ChrRecord.Data.HumItems) do begin
                UserItem:=@ChrRecord.Data.HumItems[ii];
                if UserItem.wIndex = 0 then Continue;
                 if MakeIndex =UserItem.MakeIndex then begin
                     new(ppTItems) ;
                     ppTItems.sCharName:=ChrRecord.data.sChrName;
                     ppTItems.ItemMakeIndex:=  UserItem.MakeIndex;
                     ppTItems.ItemId:=UserItem.wIndex;
                     charItmeslist.Add(ppTItems);
                 end;
            end;

            ////
            for ii:=Low(ChrRecord.Data.StorageItems) to High(ChrRecord.Data.StorageItems) do begin
                UserItem:=@ChrRecord.Data.StorageItems[ii];
                if UserItem.wIndex = 0 then Continue;
                 if MakeIndex =UserItem.MakeIndex then begin
                     new(ppTItems) ;
                     ppTItems.sCharName:=ChrRecord.data.sChrName;
           ...
           for ii := 0 to  ChrRecord.Data.m_bMaxBagitem-1 do begin
                 UserItem:=@ChrRecord.Data.BagItems[ii];
                 if UserItem.wIndex = 0 then Continue;
                 if MakeIndex =UserItem.MakeIndex then begin
                     new(ppTItems) ;
                     ppTItems.sCharName:=ChrRecord.data.sChrName;
            ...
            for ii:=Low(ChrRecord.Data.monStorageItems) to High(ChrRecord.Data.monStorageItems) do begin
                UserItem:=@ChrRecord.Data.monStorageItems[ii];
                if UserItem.wIndex = 0 then Continue;
                 if MakeIndex =UserItem.MakeIndex then begin
                     new(ppTItems) ;
                     ppTItems.sCharName:=ChrRecord.data.sChrName;
          ...
          end; // if ChrRecord.data.sChrName<>''
          Application.ProcessMessages;
          if Application.Terminated then begin
            Close;
            exit;
    ...
    end;  // if fHUMStream <> nil
    if fHUMStream<>nil then fHUMStream.Free;


end;

procedure TFileDB.FindItemByItemName(sItemName:string;var charItmeslist:Tlist);
 var  I,ii:integer;
      fHUMStream:tStream;
      humcount:integer;
      DBHeader:TDBHeader;
         ...
         if  fHUMStream.Read(ChrRecord,SizeOf(THumDataInfo)) <> SizeOf(THumDataInfo) then break;
          if ChrRecord.data.sChrName<>'' then begin
            for ii:=Low(ChrRecord.Data.HumItems) to High(ChrRecord.Data.HumItems) do begin
                UserItem:=@ChrRecord.Data.HumItems[ii];
                if UserItem.wIndex = 0 then Continue;
                 if sItemName =GetStdItemName(UserItem.wIndex) then begin
                     new(ppTItems) ;
                     ppTItems.sCharName:=ChrRecord.data.sChrName;
                     ppTItems.ItemMakeIndex:=  UserItem.MakeIndex;
                     ppTItems.ItemId:=UserItem.wIndex;
...
function TFileDB.CleardupItem():integer;
 var  I,ii:integer;
      fHUMStream:tStream;
      humcount:integer;
      DBHeader:TDBHeader;
...
begin
    ...
    if fHUMStream <> nil  then   begin
     // FileRead(humFileHandle,DBHeader,SizeOf(TDBHeader));
         ...
         if  fHUMStream.Read(ChrRecord,SizeOf(THumDataInfo)) <> SizeOf(THumDataInfo) then break;
          if ChrRecord.data.sChrName<>'' then begin
            for ii:=Low(ChrRecord.Data.HumItems) to High(ChrRecord.Data.HumItems) do begin
                ...
                if UserItem.wIndex = 0 then Continue;
                 new(ppTItems) ;
                 ppTItems.charnameid:=i;
                 ppTItems.ItemMakeIndex:=  UserItem.MakeIndex;
                 ppTItems.iteminbagtype:=0;
                 ppTItems.iteminbagpos:=ii;
                 charItmeslist.Add(ppTItems);
            end;
       ...
       //这里已经把数据全部放入 charItmeslist;
        charItmeslist.Sort(@comp); //这里排序
        ProcefinddupItem(charItmeslist,dupItemlist);
        result:=dupItemlist.Count;

        if result=0 then  Application.MessageBox(PChar('没有发现复制品！'),PChar('提示'),MB_ICONQUESTION)
        else begin
          s:=format('使用%d 毫秒 找到 %d 个复制物品,是否确定清除复制品',[GetTickCount-T,result]);
          if Application.MessageBox(PChar(s),PChar('提示'),MB_YESNO + MB_ICONQUESTION) = mrYes then begin

           ProcecleardupItem(dupItemlist) ;
           Application.MessageBox(PChar('复制品清除完成！，请查看“清除复制品列表.txt”'),PChar('提示'),MB_ICONQUESTION);
          end;
        end;
    end;  // if fHUMStream <> nil
    if fHUMStream<>nil then fHUMStream.Free;
    for I:=0 to charItmeslist.Count-1 do begin
        dispose(pTItems(charItmeslist.Items[i]));
    end;
    charItmeslist.Free;
