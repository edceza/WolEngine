...
uses
  ...
  Controls, Forms, Dialogs, StdCtrls, ExtCtrls, Buttons,Grobal2, Spin;
type
...
begin
   key:=#13 ;
   EdFindKeyPress(sender,key);
end;

procedure TFrmFDBExplore.Button3Click(Sender: TObject);
var T,n:dword;
    s:string;
begin
//  MessageDlg('«Î±£÷§∑˛ŒÒ∆˜»ÀŒÔ∂ºœ¬œﬂµƒ«Èøˆœ¬ π”√øÏÀŸ«Â≥˝∏¥÷∆£¨ «∑Ò»∑»œºÃ–¯£ø',mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
 if  Application.MessageBox(PChar('«Î±£÷§∑˛ŒÒ∆˜»ÀŒÔ∂ºœ¬œﬂµƒ«Èøˆœ¬ π”√øÏÀŸ«Â≥˝∏¥÷∆£¨ «∑Ò»∑»œºÃ–¯£ø'),PChar('Ã· æ'),MB_YESNO + MB_ICONQUESTION) = mrYes then begin

     boAutoClearDB:=False;
    // HumDataDB.Rebuild();

     n:= HumDataDB.CleardupItem;
   //  MessageDlg('øÏÀŸ«Â≥˝∏¥÷∆ÕÍ≥…£° π”√ ±º‰£∫'+INTtOSTR(GetTickCount-T)+'’“µΩ'+inttostr(n)+'∏ˆ∏¥÷∆ŒÔ∆∑',mtInformation, [mbOK], 0);
  {   s:=format('øÏÀŸ«Â≥˝∏¥÷∆ÕÍ≥…£¨ π”√ ±º‰£∫%d ∫¿√Î ’“µΩ %d ∏ˆ∏¥÷∆ŒÔ∆∑, «∑Ò»∑∂®«Â≥˝∏¥÷∆∆∑',[GetTickCount-T,n]);
    if Application.MessageBox(PChar(s),PChar('Ã· æ'),MB_YESNO + MB_ICONQUESTION) = mrYes then begin
