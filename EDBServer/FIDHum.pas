...
uses
  ...
  Controls, Forms, Dialogs, StdCtrls, DB, DBTables, Grids, Buttons,HumDB,Grobal2,localDb;
type
  TFrmIDHum=class(TForm)
    ...
    procedure RefItemGrid(n08:Integer;SeachItem: TseachItems);
    ...
    procedure ItemsGridClick(Sender: TObject);
...
begin
  Edit1.Text:='';
  Edit2.Text:='';
  ItemsGrid.Cells[0,0]:='物品名称';
  ItemsGrid.Cells[1,0]:='角色名称';
  ItemsGrid.Cells[2,0]:='物品序列号';
    ...
    end;
 {
     try
      if HumDataDB.Open then begin
        ...
        HumDataDB.Get(nIndex,HumRecord);
        HumRecord.boDeleted:=True;
...
begin
  ...
  if HumDBRecord.boDeleted then
    ChrGrid.Cells[4,nRowCount]:=DateTimeToStr(HumDBRecord.UpdateDate)
...
procedure TFrmIDHum.Button2Click(Sender: TObject);
var key:char;
begin
 key:=#13;
EdUserIdKeyPress(sender,key);
end;




procedure TFrmIDHum.Button3Click(Sender: TObject);
var chaitemslist:Tlist;
    SeachItem: TseachItems ;
    ppSeachItem :pTseachItems  ;
    I:integer;
begin
if  Application.MessageBox(PChar('查找物品将要搜索整个数据库，可能需要一些时间，是否确认继续？'),PChar('提示'),MB_YESNO + MB_ICONQUESTION) = mrYes then begin
    if EditItemName.Text='' then exit;
   ItemsGrid.RowCount:=1;
   chaitemslist:=Tlist.Create;
   HumDataDB.FindItemByItemName(EditItemName.Text,chaitemslist);
   for I:=0 to chaitemslist.Count-1 do begin
    SeachItem:= pTseachItems(chaitemslist.items[i])^;
    RefItemGrid(i,SeachItem);
   end;
   for I:=0 to chaitemslist.Count-1 do begin
    dispose(pTseachItems(chaitemslist.items[i])) ;
   end;
  chaitemslist.Free;
end;
...
procedure TFrmIDHum.RefItemGrid(n08: Integer; SeachItem: TseachItems);
var  nRowCount:integer;
begin
  ItemsGrid.RowCount:=ItemsGrid.RowCount+1 ;
  ItemsGrid.FixedRows:=1;
  nRowCount:=ItemsGrid.RowCount - 1;
  ItemsGrid.Cells[0,nRowCount]:=GetStdItemName(SeachItem.ItemId);
  ItemsGrid.Cells[1,nRowCount]:=SeachItem.sCharName;
  ItemsGrid.Cells[2,nRowCount]:=inttostr(SeachItem.ItemMakeIndex);
end;

procedure TFrmIDHum.ItemsGridClick(Sender: TObject);
var
  nRow:Integer;
begin
  nRow:=ItemsGrid.Row;
  if nRow < 1 then exit;
  if ItemsGrid.RowCount - 1 < nRow then exit;
  EdChrName.Text:=ItemsGrid.Cells[1,nRow];
end;

procedure TFrmIDHum.Button4Click(Sender: TObject);
var chaitemslist:Tlist;
    SeachItem: TseachItems ;
    ppSeachItem :pTseachItems  ;
    I,MakeIndex:integer;
...
begin
if  Application.MessageBox(PChar('查找物品将要搜索整个数据库，可能需要一些时间，是否确认继续？'),PChar('提示'),MB_YESNO + MB_ICONQUESTION) = mrYes then begin
    if EditItemMakeIndex.Text='' then exit;
    MakeIndex:=Str_toint(EditItemMakeIndex.Text,-1) ;
    if MakeIndex=-1 then exit;
   ItemsGrid.RowCount:=1;
   chaitemslist:=Tlist.Create;
   HumDataDB.FindItemByIndex(MakeIndex,chaitemslist);
   for I:=0 to chaitemslist.Count-1 do begin
    SeachItem:= pTseachItems(chaitemslist.items[i])^;
    RefItemGrid(i,SeachItem);
   end;
